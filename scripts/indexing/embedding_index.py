...
def load_dotenv(path: Path) -> None:
    ...
    _load_env_file(path, strict=True)
...
def require_env() -> str:
    ...
    return require_database_url(
        ENV_PATH,
        loader=load_dotenv,
        error_message=f"DATABASE_URL is required; set it in the environment or {ENV_PATH.name}",
    )
...
def index_chunks(
    ...
    """
    if __package__.startswith("scripts."):
        from ..embeddings.embedding import build_embedding_text  # noqa: PLC0415
    else:
        from embeddings.embedding import build_embedding_text  # noqa: PLC0415
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    if __package__.startswith("scripts."):
        from ..embeddings.model_registry import SUPPORTED_ALIASES  # noqa: PLC0415
    else:
        from embeddings.model_registry import SUPPORTED_ALIASES  # noqa: PLC0415
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if __package__.startswith("scripts."):
            from ..embeddings.model_registry import resolve_model_config  # noqa: PLC0415
        else:
            from embeddings.model_registry import resolve_model_config  # noqa: PLC0415
